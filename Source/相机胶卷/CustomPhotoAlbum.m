...
- (void)certificateCollectionViewCellDelegateEventType:(TouchEventType)touchEventType atIndex:(NSInteger)index {
    ...
    switch (touchEventType) {
        case TouchEventTypeSelect: { // 选中状态 albumModelArray
            // 9张图片的限制
            if (self.modelAdditionArray.count >= self.photoNum) {
        ...
        case TouchEventTypePhoto: { // 进入拍照
            ...
            [AlbumTool cameraAuthorizationWithAuthorization:^(CameraAuthorizationType authorization) {
                ...
                if (authorization == CameraAuthorizationTypeClose) {
                    ...
                    customCameraVC.photoNum = self.photoNum;
                    customCameraVC.cameraPhotoImagesArray = self.cameraPhotoImagesArray;
...
- (UIImage *)scaleFromImage:(UIImage *)image {
    ...
    CGFloat dataSize = data.length/1024.0;
    ...
    if (dataSize <= 1) {
        ...
        size = CGSizeMake(width/dataSize, height/dataSize);
    }
    ...
    NSData *dataOne = UIImagePNGRepresentation(newImage);
//    CGFloat dataSize = data.length/1024;
    NSLog(@"dataOne ******* %lu",dataOne.length/1024.0);
...
- (void)customPhotoAlbumPreviewCompleteButtonClickWithDeleteModel:(NSMutableArray *)deleteModelArray {
    ...
    [self updateEnsureButtonContent];
